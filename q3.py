...
def compute_saliency_maps(X, y, model):
    ...
    loss = correct_scores - tf.sqrt(tf.reduce_sum(model.image**2, axis = [1, 2, 3]))
    grads = tf.gradients(loss, model.image)
    tsaliency = tf.reduce_max(tf.abs(tf.reshape(grads, X.shape)), axis = 3)
    saliency = sess.run(tsaliency, feed_dict={model.image:X, model.labels:y})
    print(loss.shape)
    print(X.shape)
...
def make_fooling_image(X, target_y, model):
    ...
    for iters in range(20):
        print(iters)
        correct_score = model.classifier[0, target_y]
        loss = correct_score
        tgrad = tf.gradients(loss, model.image)[0]
        dX = tf.div(learning_rate * tgrad, tf.norm(tgrad))
        dX = sess.run(tgrad, feed_dict={model.image:X_fooling, model.labels:ty})
        X_fooling += dX
...
print(scores[0].argmax() == target_y)
...
def create_class_visualization(target_y, model, **kwargs):
    ...
    loss = model.classifier[0, target_y] - l2_reg * tf.square(tf.norm(model.image[0]))
    ...
    for t in range(num_iterations):
        ...
        ########################################################################
        pass
        X += sess.run(grad, feed_dict={model.image:X, model.labels:[target_y]})
